...
Untuk semetara, kita hanya membuat resolvers untuk `Query` saja dengan dua query yaitu `hello` dan `books`. Data di dalam `books` adalah data dummy yang kita gunakan untuk contoh.

### Membuat Apollo Server

Setelah membuat schema dan resolvers, kita akan membuat Apollo Server yang akan digunakan oleh aplikasi SvelteKit. Server ini akan dapat diakses melalui endpoint `/graphql`. File server ini akan disimpan pada folder `src/routes/graphq/+server.ts`.
...
Kode awal adalah beberapa `import` dan `interface` yang akan digunakan oleh Apollo Server. Abaikan jika tidak memahami kode tersebut. 
...
Kode berikutnya adalah kode untuk menangani request yang masuk ke Apollo Server. Kita menggunakan `POST` method untuk menangani request yang masuk. Kita menggunakan `executeHTTPGraphQLRequest` untuk mengeksekusi query yang dikirimkan oleh aplikasi SvelteKit.
